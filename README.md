...
Temperature Monitor reads temperatures from DS18B20 sensors in grain bins. It consists of a server and multiple clients (controlboxes). The server is installed on a PC on the LAN. Each controlbox sends data over wifi to the server. On startup if a network connection can't be made a web page is started at 192.168.4.1 . 
